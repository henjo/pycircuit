...
`gm·R = 0.5`, 4e-21 A²/Hz per stage, f₀ = 1 MHz, gear, 400 points:

| | σ_t at the last crossing | notes |
|---|---|---|
| sources ON | **5.712532e-11 s** (5.71e-05 of T) | band [1 kHz, 500 kHz] |
| sources OFF | 3.669792e-12 s | the method's own floor — **242× down in variance** |
| PSD × 4 | 1.140737e-10 s | ratio **1.99689** against 2.000 predicted for σ_t ∝ √PSD |

⚠ The OFF floor is 6.4 % of the figure in σ_t and is **quoted beside it, never subtracted** — that is exactly the
manufactured floor Biggio et al. warn of, which A8's entry requires a sources-off control for.

✅ **SECOND FALSIFIER, on an axis independent of the PSD one.** Drive amplitude only (`gm`, loads, sources fixed),
so the noise transfer is untouched and only the edge moves: over a 7.8× slew range the product σ_t·slew held at
1.1834 / 1.1916 / 1.1938 / 1.1943 e-03 V — **σ_t ∝ 1/slew to 0.9 %**.

⚠⚠ **A FALSIFIER I HAD TO WITHDRAW, because the experiment could not test its own prediction.** The first slew
check varied `gm` in ALL THREE stages, which moves the cascade gain as ~`(gm·R)³`: swing collapsed 0.875 → 0.221 →
0.033 and slew fell 7.87× / 7.98× per step, so the 4.33 / 6.76 ratios refuted a prediction about a quantity the
knob never isolated. Varying the DRIVE is the clean axis.

**Observation, deliberately not a claim:** σ_t is flat down the chain — 5.576e-11 / 5.871e-11 / 5.713e-11 s at
o0/o1/o2 — because variance (2.75e-7 → 6.79e-7 → 1.40e-6 V²) and slew (9.41e6 → 1.40e7 → 2.07e7 V/s) grow
together. Whether that is "does not accumulate" needs the per-stage contributions separated, which was not done.
