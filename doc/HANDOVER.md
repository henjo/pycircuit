...
| Events phase B: the bordered sideband response (PAC on a staged solve) | ⚠ **DESIGNED AND DRAFTED 2026-09-22, NOT VERIFIED, REVERTED.** Block elimination with the event rows (`events_pac_patch.py` in scratch); the FD instrument never passed its linear-circuit control (RMS/energy-folded output, envelope 3× off) — start the next attempt from PAC's own analytic-transfer-function fixture. Open with it: the bordered Lyapunov closure (noise), gear's columns, autonomous circuits with events. |
