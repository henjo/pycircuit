...
def resid(x):
    ...
    ev = np.linalg.eigvals(V); ev = ev[np.argsort(-np.abs(ev))]
    res += [abs(ev[0] - 1.0)] + list(np.abs(ev[1:]))
    Mi = V - B @ np.linalg.solve(A, U)
    res += list(np.abs(np.linalg.eigvals(Mi)) * 3.0)             # epsilon = 0: L-stable
