...
| `Transient` at STATE events (E7) | ⏳ **OPEN 2026-09-22** (Andreas: on the list): the transient lands source events only; a comparator crossing is resolved by LTE rejection (24–99 steps per crossing at 1e-9 on the sharp oscillator — right to 0.45 % of the exact PPV, but expensive, and first order at looser tolerances when a step straddles it). Build: land `state_events()` rows inside a step, restart the history, expose the times; gate against `_exact_relaxation_oscillator_ppv`. |
