        ...
        for trial in range(3):
            def resid(x):
                try:
                    ...
                    return np.ones(r * r + 2 * r + r * (r - 1) // 2 + r) * 1e3
                ...
                ## DOMAIN bound only -- c in [0, 1], no magnitude objective (the two were
                ## introduced together the first time, which is a two-factor change scored as one)
                res += [max(cc[i] - 1.0, 0.0) * 30 + max(0.0 - cc[i], 0.0) * 30 for i in range(r)]
                return np.array(res)
...
print('max|A| %.3f  max|B| %.3f  c in [%.4f, %.4f]' % (np.max(np.abs(m['A'])), np.max(np.abs(m['B'])), min(c), max(c)))
