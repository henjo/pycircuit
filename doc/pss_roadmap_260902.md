...
✅✅ **STEP 3 DONE THE SAME DAY — THE LOCK RANGE, AND AS A LOCUS.** The hold-in range is the SAME constant steps 2 and 4 measured (`Df_max = kvco*K/2`), so this is a **third independent route to one number** from a third physical quantity. And the whole curve is predicted: `-ln|lam|/(pi kvco K T) = s + s^2 (f_c/f_RC)` with `s = sqrt(1 - (Df/Df_max)^2)` — a unit semicircle tilted by the RC — matching to **6.3e-07 .. 7.9e-06** over x = 0..0.99 with NOTHING fitted, and lock lost between 99 and 100.5 Hz. ⚠ Step 4's second-pole term is what closed it: without it the locus matched only to 6.3e-4, and that residual read exactly `f_c/f_RC` at zero detuning. Test `test_the_pll_lock_range_is_the_loop_bandwidth_and_the_whole_locus_is_predicted`. **A6's ANALYSIS half is now done (lock range + phase noise); what remains is the PFD or a phase-domain detector, and realistic divider ratios, which wait on A5.**

✅✅ **STEP 4 DONE THE SAME DAY — THE LOOP SHAPES ITS VCO'S PHASE NOISE, against a CLOSED FORM with nothing fitted.** `S_phi,closed = sf (1 + 2 f_c/f_RC) / (4 pi^2 (f_m^2 + f_c^2))`, agreeing to ~1e-4 over three decades of offset and grid-independent to eight digits. **Neither constant is fitted**: `f_c` comes from the FLOQUET MULTIPLIER (100.063 Hz against `kvco*K/2` = 100.000, a quantity that never looks at noise) and `2 f_c/f_RC` from R and C. ⚠ The `4 pi^2` is the CYCLES convention — `VcoHdl`'s docstring quotes the RADIAN form `sf/f_m^2`, and applying it to the `ph` node reads 39.5x low; it was caught because the error was a CONSTANT across every offset, and *a flat scale error is units, a shape error would be physics*. ⚠ The second-pole term was FOUND, not assumed: a constant 1.2586e-3 survived, was NOT discretisation (identical at npts 400/800/1600) and NOT the first pole (identical below and far above the corner), and scales linearly on two independent knobs. Test `test_a_locked_loop_shapes_its_vco_phase_noise_with_nothing_fitted` (6 s). So A6's remaining work is the PFD or a phase-domain detector and lock detection; the noise half is demonstrated at small N.
...
⚠ **B7's open question — can any LTE-based controller see an accumulating period error? — and one
paper NOT to re-read for it** (peer `docs-46`, 2026-09-07). Römisch & Winkler, *"Stepsize control
for mean-square numerical methods for stochastic differential equations with small noise"*, SIAM J.
Sci. Comput. 28(2):604–625 (2006), surfaced on the LTE/step-control axis with 31 hits and zero
out-of-scope. ❌ **Null, and it is the main result:** it controls stepsize by estimating the p-th
*mean of local errors of an SDE integration* — stochastic local-error control. It does not treat
the deterministic frequency-warping error and never discusses the accumulation-vs-LTE distinction
that makes warping invisible (fact (a), Brambilla & Storti-Gajani, verified). Not evidence either
way on whether LTE-blindness is a class limitation or an artefact of these estimators. **B7's
question stays open.** The better candidate is already on disk and partly read: Kennedy &
Carpenter's DIRK review, whose own complaint is that *"many published schemes do not include error
...
#### ✅ An unlooked-for positive on A8's gate — a published stepsize strategy built for the failure mode it warns about

⚠ Relayed by `docs-46` from Römisch & Winkler, SIAM J. Sci. Comput. 28(2):604–625 (2006), found
while checking it for B7 (where it is a null — see B7). Biggio et al.'s floor has two mechanisms;
the second, verbatim: *"the linear multi-step integration method uses a variable-length time step.
This 'warps' time and introduces errors … the solutions change if different (non uniform) time
grids are used."* Römisch & Winkler's central design property answers exactly that one: *"The
strategy leads to stepsize sequences that are identical for all computed paths"*, and *"a stepsize
sequence generated by Algorithm 3.1 is deterministic, since `χ(x_{l−1}, x_l, t_{l−1}, t_l)‖_{L^p}`
is deterministic, though `x_{l−1}, x_l` are random variables."* **A deterministic, path-independent
grid removes the path-to-path grid variation that turns grid sensitivity into apparent noise.** So
A8's gate — until now a warning plus a sources-off control — has a published stepsize strategy
designed for what it warns about, with computable estimates of the dominating local-error term for
the Euler family and the θ-methods (trapezoidal at `θ = ½` analysed explicitly).

⚠ **Two scope limits, both the authors':** the estimates are for SDEs with **small noise** and the
Euler/θ families — **not the stage methods this tree now defaults to** — and §1 notes *"SDE models
in circuit simulation that do not satisfy the commutativity condition"*, so a circuit SDE may fall
...
The record lives in **`doc/pss_log_260902.md`**: what was built and
measured, in order.  This file is the PLAN.

⚠ Code docstrings reference sections BY NAME (e.g. "the radau-default
section", "A10").  Both levels are listed here so a name-based pointer
always resolves -- if one sent you to this file, find it below and follow
it to the log.  42 top-level sections, 86 subsections.
