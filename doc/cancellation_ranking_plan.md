...
**Status: COMPLETE — every declared stage has a recorded outcome. Read
§"Outcomes" first; every number there comes from a logged run of the script named
beside it.**

**The headline, in one paragraph.** Convergent device-symbolic approximation works
and is shipped (`DDD.approximate_groups`), with two cheap diagnostics
(`DDD.cancellation`, `DDD.concentration`) and their measured limits. A *readable*
result was reached for one coefficient (11 operations at 26% error) and for a
transfer function over two decades at one operating point (177 operations at 7.9%).
It was **not** reached over the amplifier's real band, where the best is ~4 000
operations — roughly 20× too large. Stage 13 explains why no listed lever closes
that gap: `Short`, element pruning, de-cancellation and GPDD all attack *term
supply*, and supply is not the binding constraint — the accuracy demanded across
the frequency band is.

**Scope note.** The *nonlinear* work on the leapfrog that this document briefly
carried has been moved to where it belongs — `doc/distortion_ddd_plan.md` §7 and
`doc/distortion_ddd_conclusions.md` §10, with the live table in
`doc/src/circuit/distortion_ddd.rst`. It was never this plan's subject, which is
term ranking.
