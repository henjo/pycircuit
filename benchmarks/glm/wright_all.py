...
for perm in itertools.permutations(range(p)):
    print('perm %s   best so far %.3e   [%.0f s]' % (perm, best[0] if best else float('inf'), _time.time() - _t0), flush=True)
        ...
        for trial in range(3):
            def resid(x):
                try:
                    ...
                    return np.ones(r * r + 2 * r + r * (r - 1) // 2 + r + 2 * r * r) * 1e3
                res = list(np.triu(m['Atil']).ravel())
                res += list(m['B'][0] - m['A'][-1])
                res += list(m['B'][1] - np.eye(r)[r - 1])
                cc = np.array(c)
                res += [max(0.02 - abs(cc[i] - cc[j]), 0.0) * 30 for i in range(r) for j in range(i)]
                ## BOUND the abscissae to [0, 1]: a stage at c > 1 evaluates the device models past
                ## the end of the step and samples a time-dependent source in a DIFFERENT segment;
                ## c is a free parameter (Wright p.80) and his canonical choice is inside.
                res += [max(cc[i] - 1.0, 0.0) * 30 + max(0.02 - cc[i], 0.0) * 30 for i in range(r)]
                ## and a soft pull on coefficient magnitude -- 3.11: methods with all coefficients
                ## <= 1 exist at high order, and the construction is LINEAR in the free parameters
                res += [0.02 * max(abs(v) - 1.0, 0.0) for v in np.concatenate((m['A'].ravel(), m['B'].ravel()))]
                return np.array(res)
...
print('max|A| %.3f  max|B| %.3f  c in [%.3f, %.3f]' % (np.max(np.abs(m['A'])), np.max(np.abs(m['B'])), min(c), max(c)))
