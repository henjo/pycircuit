...
## 27-29 moved: the nonlinear leapfrog work

The three sections that were here — the feasibility of fully symbolic nonlinear
analysis on the 127-unknown leapfrog, the order-versus-amplitude table, and the
correction about the cubic's turning point — **have moved to
`doc/distortion_ddd_conclusions.md` §10**, with the plan stages in
`doc/distortion_ddd_plan.md` §7 and the live table in
`doc/src/circuit/distortion_ddd.rst`.

They were written here because that is where the session happened, not because they
belong here: this document is about **term ranking and cancellation** in a linear
determinant, and that work is about **perturbation order** in a nonlinear analysis.
The one genuine connection is worth keeping in view, and it is recorded at the end of
§10 there: the two goals want *opposite* representations. An expanded, rankable form
can be read and cannot scale; a hash-consed straight-line program scales beautifully
and cannot be read. Every stage of §§1-26 above was spent trying to make the first
