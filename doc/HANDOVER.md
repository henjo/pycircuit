...
| A8 sampled / edge-jitter noise | OPENED 2026-09-16, reconnaissance measured, still unbuilt. ⚠ "point `sampled_noise` at the crossings" is WITHDRAWN — it refuses autonomous PSS by design. Oscillator share = `c` (n-scaling reproduces it to 1.0005). ✅ A first ADDITIVE number needs no new machinery: the buffer chain is DRIVEN given the waveform — 3 tanh buffers, gm·R=0.5, PSD 4e-21, f0=1 MHz → σ_t 5.712532e-11 s, band [1e3, 5e5]; sources-OFF floor 3.669792e-12 s (242× down, quote it, never subtract); falsifiers PSD×4 → 1.99689 (predict 2.000) and σ_t·slew constant to 0.9 % over 7.8× slew. ⚠ OUT OF SCOPE: LDO delay modulation (bias-dependent, non-stationary) and ρ_k across periods → Demir 1996. Full record + eight convention errors in `doc/pss_log_260902.md` "A8 opened" |
