...
| ~~`lte_grid`/`refine_grid` step with the Transient default (`Gear2Integrator()`) whatever the PSS's method~~ (2026-09-21) | ✅ **BUILT 2026-09-21.** Both adaptive runs pass `integrator=self._integrator_for(self.par.method)`. Each method on its own grid (μ = 10): gear 195 pts −22 / −1461 ppm (two windows — gear's period on a frozen adaptive grid depends on where the window's fine regions sit against the converged edges; not the seam or ramp, measured), trbdf2 286 pts −8.7, radau 110 pts −0.32. The `method` table carries the guidance: trbdf2 or radau on a relaxation oscillator's adaptive grid; reltol tighter than 1e-5 above μ ~ 10; seed `solve` from `pss.lte_period`. |
