...
| `safe_ln(x)` | `ln(hypsmooth(x))` | no `−inf` at zero; **squares its argument** — see the 2026-08-24 correction |
...
`hypsmooth` overflows, and buying that range back would cost a comparison
on every evaluation of the most frequently called function in the kernel.

> **Correction (2026-08-24): "circuit quantities do not reach there" is
> false, and the cost of these helpers needs stating.**
>
> They do reach there, and not only at absurd bias. `safe_ln` goes
> through `hypsmooth`, so it **squares its argument** and carries the
> same `1e154` ceiling — and `ln((1 + Dsi)/(1 + Dgate))` in PSP's gate
> tunnelling reaches `Dgate = 4.4e222` at `Vds = 1e12`. The drain
> current came back `-inf` from an expression every ingredient of which
> was finite.
>
> **Every regulariser in the table above spends exponent range.**
> `safe_div` squares its `eps`, `hypsmooth` squares its, `safe_abs`
> squares its, and `safe_ln` inherits `hypsmooth`'s. That is the right
> trade when the guard can fire and a pure loss when it cannot: the two
> arguments here were `1 + expl(...)`, at least 1 by construction, so
> the guard could never fire and the range it cost was real. **Ask what
> a `safe_` helper is guarding against and whether that case is
> reachable; if the argument is provably positive, plain `log` is both
> correct and wider.**
>
> Two related facts, both measured in the same block:
>
> * **a PRODUCT of two `expl` outputs is not bounded.** `expl` is finite
>   for every double, but past its seam it continues *polynomially*
>   rather than saturating, so each factor reaches ~1e186 at
>   `|Vds| = 1e27` and the product overflows. Where only the logarithm
>   of such a product is needed, carry the exponents and never form it:
>   `log(1 + e^z)` is bounded by `|z| + 0.7`, written branch-free as
>   `max(z,0) + log(1 + exp(-|z|))` so the exponential only ever sees a
>   non-positive argument;
> * **`expl_low` is a bare `exp` ABOVE its seam** — that is its
>   definition, it guards the lower tail only — so an unselected arm
>   feeding it a large positive argument overflows. `Min(t, 0)` puts it
>   back on the side it is valid for.
>
> And a smoothing CONSTANT needs a scale. `hypsmooth(x, 1e-3)` where the
> result is divided by a scaled parameter `VP` depends on `e/VP`, and
> `VP` is 0.322 on one card and 7.38e-6 on another — a factor of 44000.
> An absolute constant encodes an assumption about a parameter you do
> not control. Write `1e-3 * vp`, and pick the LOOSEST value that has
> converged rather than the tightest that runs: a wider smoothing is a
> rounder corner and a better-behaved Jacobian.
>
> None of this was reachable by setting the block's own parameter to
> zero. The element compiles from **symbolic** parameters, so
> `if param != 0` is true at build time whatever the value: the block is
> in the expression regardless, zero only multiplies the result at the
> end, and `0 × inf = NaN`. **A term that is "off" still has to be
> finite**, and zeroing a coefficient is not a valid test of whether a
> block causes a numerical failure — compare against a build without the
> code.
...
**Provenance of the golden data.** The compact-model comparisons are
against `pycircuit/circuit/tests/data/psp103_ihp_iv.json`, generated by
`benchmarks/psp_reference.py` from the vendor's own compiled binaries.
Two things about it are load-bearing and neither is obvious:

* **the generator's solver tolerances are part of the reference.** Run
  at ngspice's defaults it carried up to 9.6e-4 of relative error — for
  a long time the largest error in the whole comparison, and it presents
  as a KINK at one bias rather than as an offset. The sweep decks now
  set `.options reltol=1e-6 abstol=1e-15`; the `op` decks deliberately
  do not need it. See the 2026-08-24 entry in §9;
* **regeneration must be checked as ADDITIVE.** Adding outputs must
  leave every pre-existing value bit-identical, and a deliberate
  accuracy change must move only what it should. Both regenerations this
  campaign were verified that way — zero non-sweep values changed, which
  is what made a change to 411 sweep points safe to accept.
