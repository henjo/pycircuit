...
The channel-shortening logarithm needs :math:`V_{ds} - \psi_\Delta`
floored at zero — :math:`\Delta L` is a channel *shortening* and a
negative one is meaningless. PSP does not floor it; it forms

.. math::

   s_1 = \ln\frac{1 + (V_{ds} - \psi_\Delta)/V_P}
                   {1 + (V_{dse} - \psi_\Delta)/V_P}

directly and relies on the terminal ordering. The clamp is ours, added
so the factor stays even under the source/drain exchange, so its
**width** is ours to get right. It was ``hypsmooth(x, 1e-3)``: an
absolute constant, in volts.

But the smoothed quantity is divided by :math:`V_P` and put through a
logarithm, so what the answer depends on is :math:`e/V_P` — and
:math:`V_P` on this PDK is **0.322 on the n-channel and
7.38 × 10⁻⁶ on the p-channel**, a factor of 44 000. One constant was
0.3% of one scale and 135× the other. That was the whole of the
remaining p-channel error: 2.50 × 10⁻⁴ against 5.3 × 10⁻⁶ on the
n-channel equivalent.

Making it :math:`10^{-3} V_P` takes every one of the twelve sweeps to
**median 1.000 with a spread of 0.000**. The fraction is the *knee*, not
the tightest value that works — a rounder corner is better for the
Jacobian, so the right pick is the loosest smoothing that has already
converged:

.. list-table:: Worst :math:`|\ln(\text{ours}/\text{PSP})|` over the twelve sweeps
   :header-rows: 1
...
**The search is the instructive part, because the shape lied.** The
residual grew with gate drive, was largest at low :math:`V_{ds}`,
vanished in saturation and appeared on the short device only — textbook
series resistance. It correlated with ``RS`` at 0.98; ``RS`` had exactly
the right leverage; and ``RSW2`` is nonzero on the p-channel card and
*exactly zero* on the n-channel, which would have explained the
asymmetry beautifully. All of it was wrong. Zeroing ``RS`` **and**
``RSG`` in a card read by both implementations left the disagreement
bit-identical, 0.999795 against 0.999796.

Fifteen candidates correlated above 0.93 with the residual. That is what
collinearity looks like, and it is why the best-fitting one is not
thereby the true one.

What settled it was the differential over the card — zero a term in a
copy of the model card and let *both* implementations read it. Fourteen
terms left the disagreement at exactly 100% of baseline (impact
ionisation, gate current, DIBL, interface states, mobility, ``XCOR``,
``FETA``, ``AX``, ``THESAT``, ``RSW2``); zeroing the whole ``ALP`` block
collapsed it to 5.3 × 10⁻⁷. Then arithmetic finished it with no further
runs: :math:`\Delta L = \alpha s_1` and the ``ALP1`` term also carries
:math:`s_1`, while ``ALP2``'s carries :math:`s_2`. ``ALP`` alone gives
50% and ``ALP1`` alone 54% *because they share* :math:`s_1`; ``ALP2`` is
eliminated because it does not. One quantity left.

Impact ionisation, and a reference that names its own gap
---------------------------------------------------------

One sweep was then left: ``nmos_idvd_vg0p6`` at 9.4 × 10⁻⁵ when every
other was inside 5 × 10⁻⁶. Flat to six digits up to :math:`V_d = 1.15`
and then falling, *accelerating*, to 0.999906 at 1.5 V — an exponential
onset above a threshold drain bias, at low gate drive.

This one needed no differential, because **the reference named it
itself**. PSP's avalanche current flows drain-to-bulk, so the recorded
:math:`i_b` *is* the missing quantity, and

.. math::

   \frac{\text{ours} + |i_b|}{\text{PSP}} = 0.9999994

flat at every point of the avalanche region — 152× better than the
residual it replaces, and 36× on ``nmos_idvd_vg1p2``. Nothing was
adjusted to produce that. On the 1 µm device it changes nothing, as it
must.

That is a better instrument than a card differential, and it was sitting
in the reference file the whole time. **When the reference records the
other terminals, a drain-current deficit can be identified rather than
inferred** — the same move that showed the p-channel long-device
residual to be gate current, at :math:`-0.49 \times i_g` across a whole
sweep.

The block is six lines (`module:1362-1369`) and its parameters now match
PSP's own ``lp_a1``–``lp_a4`` at four geometries on both channel types.
Two of its properties are worth stating because they change what the
model guarantees:

*The avalanche is not antisymmetric, and must not be.* It flows from the
**high** terminal to the bulk, so with it on the drain current is no
longer an odd function of :math:`V_{ds}` — in PSP either. The
odd-function property belongs to the channel, and the tests that guard
it now switch the avalanche off to isolate it; with ``A1 = 0`` the
element is odd to 2 × 10⁻¹⁵ on both channel types, unchanged.

*The avalanche does not saturate, and must not.* :math:`m_{avl}` tends
to :math:`A_1 \Delta V_{sat}` as the drain bias grows, so the current
rises without bound — which is what avalanche breakdown *is*. What
bounds a real device in a solver is the element's branch-voltage
limiter, not the constitutive relation.

Gate tunnelling
---------------

Four sweeps were then left above 10⁻⁵ while the other eight sat inside
2 × 10⁻⁶, and they were exactly the four with
:math:`|i_g|/|i_d|` above 2 × 10⁻⁵ — all of them **long-device**,
because ``IGINV`` scales with gate *area* while ``IGOV`` scales with the
edge. Between a 10 × 1 µm and a 1 × 0.13 µm device that is 57× against
10×, which is why gate leakage was invisible on every short-device
sweep.

The reference named this one too. Fitting a single free split
:math:`f` of the recorded :math:`i_g` per sweep gives
:math:`|f| = 0.475\ldots0.500` on all four — the gate-to-channel current
dividing evenly between source and drain. Nothing was fitted to build
the model; that number is what said the term was worth building.

Worst sweep **4.42 × 10⁻⁵ → 1.71 × 10⁻⁶**; ``nmos_long_idvd`` 88×
better.

Measured against PSP's own *component* outputs rather than against the
total — it reports ``igcs``/``igcd`` and ``igs``/``igd``, so the overlap
is their difference:

.. list-table:: Gate-current components, ours / PSP
   :header-rows: 1
...
* ``expl_low`` is a bare ``exp`` *above* its seam — that is its
  definition — and the argument reaches 3900 at :math:`V_{ds} = -100`;
* ``safe_ln`` regularises as :math:`\sqrt{u^2 + \epsilon}`, so it
  **squares its argument** and overflowed on a :math:`u` of
  4.4 × 10²²² that plain ``log`` handles. ``safe_div`` squares its
  ``eps`` for the same reason and ``hypsmooth`` squares its; each halves
  the exponent range it can carry, and paying for a regulariser that
  cannot fire is the wrong trade;
* :math:`D_{gate}` is a **product of two exponentials**, each finite at
  10¹⁸⁶ and the product not.

The block therefore carries the *logarithms* and never forms
:math:`D_{si}` or :math:`D_{gate}`: :math:`\log(1 + e^z)` is bounded by
:math:`|z| + 0.7`.

One consequence is worth stating on its own. **None of this could be
found by switching** ``IGINV`` **to zero**, because the element compiles
from *symbolic* parameters — the block is built regardless and
:math:`0 \times \infty = \text{NaN}`. A term that is "off" still has to
be finite, and a numerical off-switch is not a test that a block is
inert.
