...
| ⏳ The follow-up round (Andreas, 2026-09-24: "rewrite the test to not lose coverage but allow the code quality to improve") | **OPEN.** Rewrite the tests that force compromises, then remove the compromise: 25 calls to the `_traverse*` views; 3 spies matching the Newton closure's `__qualname__` (pins `solve`'s phases to `PSS`'s body); 19 class-level patches of `PSS` methods/constants (force the call-time `PSS` imports); `_force_plain_map` (4); 12 `fp.kind` string asserts; 2 `getsource` checks; 4 patches of `analysis.fsolve` through `shooting`; 7 private helpers imported from `shooting`; the test file's `import *` (10 names incl. `warnings` → the 49-name `__all__`). Also review the stale text the #6 agents flagged (see the log entry of 2026-09-24): `pac_jacobian` "not shipped-ready", `_C_at`/`_G_at` limit-sync, `solve_timestep`'s rescue, `_solve_twin` vs `_lyapunov_host`, `grid_error`'s `rel_change`, `_lyapunov_pieces_plain` `iq`, `_stage_injection`, `lorentzian`, `_continuous_adjoint`, the unread `PPV_DEFLATION_ITERS`; and the event stage's `_monodromy` inconsistency. |
