...
| PSP sampler: flat ~+0.1 % in the sampled FLICKER part (peer external comparison, 2026-09-15) | 🔬 **UNPARKED 2026-09-17 (Andreas) AND HANDED TO THE PRIVATE SUITE'S SESSION**, which accepted it as its own open item 0 with a purpose-built probe and **all three predictions pinned BEFORE any run**: A grid weighting of the switch-off edge → predicted to MOVE with `npts` and not be flat in frequency; B sideband truncation → predicted to move with `maxsidebands` and bite at high f; C the per-band modulated-stationary construction for a bias-modulated flicker source → predicted INVARIANT to both and to VANISH when the source does not modulate. A/B move it ⇒ numerics; C alone ⇒ a pycircuit modelling item. ⚠ **STEP 0 BEFORE ANY MECHANISM HUNT**: is it still live 23 commits of sampled-noise work past `2fd567d`, and what is the instrument's own repeatability? The flicker part is ~1.35 % of the held variance, so ~+0.1 % of it is ~1e-5 of the total and is obtained by DIFFERENCING two nearly equal numbers; `reltol` cannot move a converged answer, so its spread IS the scatter. If the excess sits inside it, "not resolvable" is the answer. ⚠ NO RESULTS YET. ⚠ What may come back here is **ratios and "moves / invariant" only** — no reference columns, and the tool stays unnamed in this repo. ⚠ My own "blocked, needs a licensed run" was TOO STRONG and is withdrawn: their probe reads the cached raws the test already uses. 🔴 **FIRST RESULTS 2026-09-18: the residual is LIVE (+0.106 %, flat in the flicker PART from 10 Hz to 1 kHz) and CANDIDATE C IS REFUTED — my prediction was that the excess vanishes without modulation; it GROWS, 55× at half clock amplitude and 185× at zero, on a row whose flicker-OFF control is cleaner than the fixture's own. A/B/C does not span the mechanism.** ⚠ My step-0 design was also weaker than specified: `reltol` 1e-10 vs 1e-12 agrees BIT FOR BIT, i.e. the knob never bound, so there is still no error bar and the axes that could give one are A and B themselves. ✅ Bounded reference-free on this side: our DC `Sfl/Vds²` varies only 0.272 % (Vg 0.8 V) / 0.047 % (Vg 1.2 V) over Vds 0.05–10 mV, SATURATES toward low Vds and CHANGES SIGN with Vg — same order as the fixture's +0.106 % but incapable of the growth. Next: their DC-bias density comparison decides whether the growth is the reference's density or the sampled path (truncation, B). ⚠⚠ **CORRECTED SAME DAY**: (a) the "stationary" row was only CLOCK-stationary — the input sine still swings, so C falls on the 0.375 row alone and "185× at zero modulation" overstates it; (b) **RETRACTED — our density canNOT account for the +0.106 %**: a DC comparison reads ours/reference = 0.99999998 at all sixteen points, so the densities AGREE and my inference confused deviation-from-`Vds²` with disagreement-with-the-reference; (c) **B is refuted on both sideband counts** (ours and the reference's), with counts above 400 being aliasing artefacts (white ratio exactly 1/3 at 800, 1/5 at 1600); (d) the residual finally has a measured bar — a same-settings re-run moves it 3e-05 against 1.06e-03, **35:1**; (e) A (grid) half-answered, flicker-OFF identical at npts 1000 and 2000. **If A is invariant too, nothing in A/B/C survives** and the object is an excess flat in frequency, invariant to both grids and both sideband counts, identical in density at DC, and GROWING as modulation falls. 💡 Sent back: if it is a CONSTANT ABSOLUTE offset the growth is normalisation, predicting part(0.75)/part(0.375) = 54.9 exactly. Record in `doc/pss_log_260902.md` "The campaign's first results" |
