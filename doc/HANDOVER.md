...
| `max_dv_step` / `max_di_step` ignored by every Runge-Kutta and GLM method (found 2026-09-23 while surveying the transient's loops for Andreas) | ⬜ **OPEN, measured:** the per-step excursion bound lives in the LMM loop and the coupled loop (a copied block) and is absent from `_run_rk_adaptive`. On a pulsed RC with `max_dv_step='auto'` (bound 0.098 V), gear and trap go from 70 to 86 steps and hold max\|Δv\| at 0.097. radau (69 → 69) and trbdf2 (71 → 71) do not change, and radau's max\|Δv\| is 0.0996, above the bound. The default method silently ignores the option. Part of the proposed one-driver transient refactor (three loops → one driver + a per-family stepper); awaiting Andreas's call. |
